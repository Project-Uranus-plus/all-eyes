...
func main() {
	fs := token.NewFileSet()
	filePath := fmt.Sprintf("D:\\桌面\\项目\\开源项目\\all-eyes\\alerter-golang\\internal\\api\\%s", handlerName)
		...
		for _, spec := range decl.Specs {
			...
			for _, v := range interfaceType.Methods.List {
				if len(v.Names) > 0 {
					...
					funcContent += "import (\n"
					funcContent += `"github.com/Project-Uranus-plus/all-eyes/internal/pkg/core"`
					funcContent += "\n)\n\n"
